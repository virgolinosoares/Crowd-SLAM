...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp)
{
	...
	mCurrentFrame.RemoveOutliers(mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);	
	//
...
int Tracking::FeatureNumberUpdate(const double area, const int factor)
{

	int keypoints = 1000*factor;
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
	...
	// remove dynamic features	
	if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        mCurrentFrame.RemoveOutliers(mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
    else
        mCurrentFrame.RemoveOutliers(mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
